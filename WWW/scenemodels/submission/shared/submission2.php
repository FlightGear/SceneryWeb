        ...
        if ($resource_rw != '0') {
            ...
            if ($_GET["action"] == "check") {  // If action comes from the mass submission script
                while ($row = pg_fetch_row($result)) {
                    ...
                    $trigged_query_rw = str_replace("INSERT INTO fgs_objects (ob_text, wkb_geometry, ob_gndelev, ob_elevoffset, ob_heading, ob_country, ob_model, ob_group) VALUES (","",$query_rw); // Removing the start of the query from the data;
                    ...
                    $pattern = "/'[(.+?)\]/s', ST_PointFromText\('POINT\((?P<long>[0-9.-]+) (?P<lat>[0-9.-]+)\)', 4326\), (?P<elev>[0-9.-]+), (?P<elevoffset>[0-9.-]+), (?P<orientation>[0-9.-]+), '(?P<country>[a-z]+)', (?P<model_id>[0-9]+), 1\)/";
