...
<?php
    ...
    // Check the presence of "action", the presence of "signature", its length (64) and its content.
    if (isset($_GET["action"]) && isset($_GET["sig"]) && (strlen($_GET["sig"]) == 64) && preg_match("/[0-9a-z]/", $_GET["sig"]) && ($_GET["action"] == "check_update")) {
        $resource_rw = connect_sphere_rw();
        ...
        if ($resource_rw != '0') {
            ...
            if ($_GET["action"] == "check_update") {  // If action comes from the mass submission script
                while ($row = pg_fetch_row($result)) {
                    $sqlzbase64 = $row[1];
