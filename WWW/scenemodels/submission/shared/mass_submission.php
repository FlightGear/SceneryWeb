        ...
        if($resource_rw != 0) {
            ...
            if (pg_num_rows($result) != 1) {
                ...
                if(!resultdel) {
                    $page_title = "Automated Shared Models Positions Pending Requests Form";
                    include '../../inc/header.php';
                    echo "<p class=\"center\">Signature found.<br /> Now deleting request with number ". $_POST["hsig"].".</p><br />";
    ...
    if((isset($_POST["submit"])) && (isset($_POST["hsig"])) && ((strlen($_POST["hsig"])) == 64) && preg_match("/[0-9a-z]/", $_POST["hsig"]) && ($_POST["submit"] == "Submit the mass import!")) {
        ...
        if($resource_rw != 0) {
            ...
            if (pg_num_rows($result) != 1) {
                $page_title = "Automated Shared Models Positions Pending Requests Form";
                include '../../inc/header.php';
                echo "<p class=\"warning\">Sorry but the request you are asking for does not exist into the database. Maybe it has already been validated by someone else?</p><br />\n";
